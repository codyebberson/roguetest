import { Panel, GUI, Rect, Colors, TileMap, Entity, Sprite, BasicMonster, ProjectileEffect, Vec2, ConfusedMonster, App, Game, RNG, MessageLog, Keys, SelectDialog, AppState, ImagePanel } from 'wglt';
...
const PLAYER_SPRITE = new Sprite(0, 96, SPRITE_WIDTH, SPRITE_HEIGHT, 2, true, undefined, 0xffcf5cff);
const ORC_SPRITE = new Sprite(64, 144, SPRITE_WIDTH, SPRITE_HEIGHT, 2, true, undefined, 0x20e64fFF);
const TROLL_SPRITE = new Sprite(64, 216, SPRITE_WIDTH, SPRITE_HEIGHT, 2, true, undefined, 0x20e64fFF);
const POTION_SPRITE = new Sprite(658, 168, SPRITE_WIDTH, SPRITE_HEIGHT, 1, true, undefined, 0xFF0000FF);
const SCROLL_SPRITE = new Sprite(738, 168, SPRITE_WIDTH, SPRITE_HEIGHT, 1, true, undefined, 0xF0F0E0FF);
...
function castLightning(item: Entity) {
    ...
    // Zap it!
    messageLog.add('A lightning bolt strikes the ' + monster.name + ' with a loud thunder!', Colors.LIGHT_BLUE);
    messageLog.add('The damage is ' + LIGHTNING_DAMAGE + ' hit points', Colors.LIGHT_BLUE);
...
function castFireball(item: Entity) {
    ...
    messageLog.add('Left-click to cast fireball, or right-click to cancel.', Colors.LIGHT_CYAN);
    game.startTargeting((x: number, y: number) => {
        ...
        game.effects.push(new ProjectileEffect(
            FIREBALL_SPRITE,
            new Vec2(player.pixelX, player.pixelY),
            new Vec2(dx, dy),
            count
        ));
        ...
        messageLog.add('The fireball explodes, burning everything within ' + FIREBALL_RADIUS + ' tiles!', Colors.ORANGE);

        for (let i = game.entities.length - 1; i >= 0; i--) {
            const entity = game.entities[i];
            if (entity.canAttack && entity.distance(x, y) <= FIREBALL_RADIUS) {
                messageLog.add('The ' + entity.name + ' gets burned for ' + FIREBALL_DAMAGE + ' hit points.', Colors.ORANGE);
...
function castConfuse(item: Entity) {
    ...
    messageLog.add('Left-click to cast confuse, or right-click to cancel.', Colors.LIGHT_CYAN);
    game.startTargeting((x: number, y: number) => {
        ...
        messageLog.add('The eyes of the ' + monster.name + ' look vacant, as he stumbles around!', Colors.LIGHT_GREEN);
        player.inventory.remove(item);
    });
}
...
function nextLevel() {
    ...
    messageLog.add('You take a moment to rest, and recover your strength.', Colors.LIGHT_MAGENTA);
    messageLog.add('After a rare moment of peace, you descend deeper...', Colors.LIGHT_RED);
...
player.xp = 0;
player.maxXp = 10;
player.onBump = function (other: Entity) {
    if (other.canPickup) {
...
const messageLog = new MessageLog(new Rect(1, -90, 100, 50));
messageLog.add('Welcome stranger! Prepare to perish!', Colors.DARK_RED);
game.gui.add(messageLog);

game.gui.add(new TopPanel(player));
game.gui.add(new BottomPanel());

game.onUpdate = function () {
    if (app.isKeyPressed(Keys.VK_I)) {
        // Show inventory
        game.gui.add(new SelectDialog(
            new Rect(16, 64, 200, 100),
            'INVENTORY',
            player.inventory,
            (choice: Entity) => {
                choice.use();
